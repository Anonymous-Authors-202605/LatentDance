...
ModelDataPathPrefix = Path(__file__).parent / "pretrained_weights"


class Wholebody:
    def __init__(self, device="cuda:0"):
        if "cuda" in device and "CUDAExecutionProvider" not in ort.get_available_providers():
            print(f"Warning: {device} requested but CUDAExecutionProvider not available. Falling back to CPU.")
            device = "cpu"
